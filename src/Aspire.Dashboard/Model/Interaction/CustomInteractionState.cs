...
public sealed class CustomInteractionState
{
    ...
    public void AddPage(int interactionId, string route, string title)
    {
        lock (_lock)
        {
            ...
            _pages = _pages.Add(new PageRegistrationState(interactionId, route, title));
        }
    }
...
public sealed record PageRegistrationState(int InteractionId, string Route, string Title);
